...
export default function Instances() {
  ...
  const handleRefreshQrcode = useCallback(async () => {
    ...
    try {
      ...
      // Lido do store, e não da variável `instances` do closure: esta foi
      // capturada no render anterior e ainda contém a lista de ANTES do fetch,
      // então a conexão só seria percebida um ciclo depois.
      const updatedInstance = useInstancesStore
        .getState()
        .instances.find((i) => i.id === qrcodeModal.instance?.id);
    ...
    // `instances` saiu das dependências: a lista agora é lida do store dentro
    // da função. Mantê-la aqui recriava este callback a cada atualização da
    // lista, o que era metade da causa do timer de auto-refresh reiniciar sem
    // parar.
  }, [qrcodeModal.instance, fetchInstances]);
